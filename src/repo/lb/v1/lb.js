...
JDCloud.LB = class LB extends Service {
  constructor (options = {}) {
...
protocol - 后端服务的协议【alb】支持Http、Tcp，【nlb&amp;dnlb】支持Tcp，默认查询所有，支持单个
  ...
  describeBackends (opts, regionId = this.config.regionId, callback) {
    ...
    let headerParams = {
      'User-Agent': 'JdcloudSdkNode/1.0.0  lb/0.4.1'
    }
...
urlMapIds - 【仅alb支持】转发规则组Id列表，支持多个
  ...
  /**
      ...
      * @param {array} [opts.certificateSpecs] - 【alb Https和Tls协议】ssl server证书列表，现只支持一个证书  optional
...
vpcId - 负载均衡所在Vpc的Id，支持单个
  ...
  /**
      ...
      * @param {chargeSpec} [opts.chargeSpec] - 【alb】支持按用量和按配置(按配置即将下线，请勿使用，已使用该计费方式的服务后续请使用按用量方式创建alb)计费，默认为按用量，目前免费。【nlb】支持按用量计费，目前免费。【dnlb】支持按配置计费，目前免费  optional
