...
JDCloud.MONITOR= class MONITOR extends Service {
    constructor(options = {}) {
            ...
            options._defaultEndpoint.host = options._defaultEndpoint.host || 'monitor.jcloud.com';
...
3: 所有规则
      ...
      * @param {string} [opts.resourceID] - 资源ID  optional 
      * @param {integer} [opts.ruleType] - 规则类型, 1表示资源监控，6表示站点监控  optional 
...
filter name 为resourceIds表示查询多个资源的规则  optional 
    ...
    createAlarm(opts, regionId = this.config.regionId, callback){
         ...
         opts = opts || {};
          ...
          if (opts.metric === undefined || opts.metric === null) {
               throw new Error('Missing the required parameter \'opts.metric\' when calling createAlarm');
          }
          if (opts.serviceCode === undefined || opts.serviceCode === null) {
               throw new Error('Missing the required parameter \'opts.serviceCode\' when calling createAlarm');
          }
          if (opts.threshold === undefined || opts.threshold === null) {
               throw new Error('Missing the required parameter \'opts.threshold\' when calling createAlarm');
          }
          if (opts.times === undefined || opts.times === null) {
               throw new Error('Missing the required parameter \'opts.times\' when calling createAlarm');
          }
    ...
    describeMetricsForCreateAlarm(opts, callback){
      ...
      * @param {string} [opts.timeInterval] - 查询的时间间隔，仅支持分钟级别，例如：1m  optional 
      * @param {string} [opts.aggrType] - 聚合方式：max avg min等  optional 
